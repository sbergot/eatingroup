...
(function (undefined) {
    ...
    Cookies._getExpiresDate = function (expires, now) {
        ...
        switch (typeof expires) {
            case 'number': expires = new Date(
                now.getTime() + expires * 1000);
            break;
        ...
        if (expires && !Cookies._isValidDate(expires)) {
            throw new Error(
                '`expires` parameter cannot be converted to a valid \
                Date instance');
        }
    ...
    Cookies._getCookieObjectFromString = function (documentCookie) {
        ...
        for (var i = 0; i < cookiesArray.length; i++) {
            ...
            var key = decodeURIComponent(
                cookiesArray[i].substr(0, separatorIndex));

            if (cookieObject[key] === undefined) {
                var value = decodeURIComponent(
                    cookiesArray[i].substr(separatorIndex + 1));
    ...
    Cookies._renewCache = function () {
        Cookies._cache = Cookies._getCookieObjectFromString(
    ...
    // CommonJS and Node.js module support.
        ...
        // But always support CommonJS module 1.1.1 spec (`exports`
